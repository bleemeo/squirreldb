...
func structsProvider(s any, tag string) *provider {
	return &provider{s: s, tag: tag}
}

// Read reads the struct and returns a nested config map.
func (s *provider) Read() (map[string]any, error) {
	ns := structs.New(s.s)
	ns.TagName = s.tag

	// Map() returns a map[string]any, but the underlying types are still known,
	...
	// to []any by marshaling and unmarshaling the map.
	m := ns.Map()
...
func (s *provider) Watch(_ func(event any, err error)) error {
